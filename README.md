...
- [Best practices](#best-practices)
- [Documentation](#documentation)
- [Logging (JSON) and how to disable or reduce it](#logging-json-and-how-to-disable-or-reduce-it)
- [FAQ / Troubleshooting](#faq--troubleshooting)
- [Compatibility](#compatibility)
- [Versioning](#versioning)
- [Contributing](#contributing)
...
## Logging (JSON) and how to disable or reduce it
By default this library emits structured JSON log lines for saga lifecycle and step execution. All loggers are enabled at INFO level so you get visibility out of the box, including:
- Saga lifecycle start/completion
- Step start/success/failure (with attempts and latency)
- Execution configuration and results per step
- Compensation lifecycle events

The default loggers are provided by:
- SagaLoggerEvents (wired by default as part of the SagaEvents composite)
- StepLoggingAspect (AspectJ-based method interceptor for @SagaStep and @ExternalSagaStep)

Disable or reduce logging:
1) Spring Boot property (disable the AOP step logging aspect)
```
transactionalengine.step-logging.enabled=false
```
2) Change log levels (Spring Boot application.yml)
```
logging:
  level:
    com.catalis.transactionalengine: WARN
```
3) Replace or narrow events
- Provide your own SagaEvents bean to replace the default logger implementation if you want to export to metrics/traces only.

Notes
- All messages are emitted as compact single-line JSON. String fields are safely escaped; long payloads are truncated.
- If you prefer fewer logs, set your package log level to WARN or ERROR; the library will honor your logging configuration.
