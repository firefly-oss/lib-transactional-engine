...
class ExecuteByClassIT {
    ...
    @Test
    void execute_with_saga_class_overload_works() {
        ctx = new AnnotationConfigApplicationContext(AppConfig.class);
        SagaRegistry reg = ctx.getBean(SagaRegistry.class);
        assertNotNull(reg.getSaga("ExecByClassSaga"));

        SagaEngine engine = ctx.getBean(SagaEngine.class);
        SagaContext sctx = new SagaContext("corr-exec-class-1");
        ...
        SagaResult result = engine.execute(Orchestrator.class, inputs, sctx).block();
        assertNotNull(result);
        assertTrue(result.isSuccess());
        assertEquals("ExecByClassSaga", result.sagaName());
        assertEquals("ExecByClassSaga", sctx.sagaName());
