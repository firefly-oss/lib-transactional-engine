...
import com.firefly.transactional.shared.annotations.Header;
import com.firefly.transactional.shared.annotations.Headers;
...
class ExecuteByMethodRefExecIT {
    ...
    @Test
    void execute_with_method_reference_overload_works() {
        ctx = new AnnotationConfigApplicationContext(AppConfig.class);
        SagaRegistry reg = ctx.getBean(SagaRegistry.class);
        assertNotNull(reg.getSaga("ExecByMethodRefSaga"));

        SagaEngine engine = ctx.getBean(SagaEngine.class);
        SagaContext sctx = new SagaContext("corr-exec-mr-1");
        ...
        SagaResult result = engine.execute(Orchestrator::c1, inputs, sctx).block();
        assertNotNull(result);
        assertTrue(result.isSuccess());
        assertEquals("ExecByMethodRefSaga", result.sagaName());
        assertEquals("ExecByMethodRefSaga", sctx.sagaName());
