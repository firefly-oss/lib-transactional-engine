...
/**
     ...
     * - Applies retry with backoff and timeout via {@link #attemptCall(Object, Method, Object, SagaContext, long, int, long, String)}.
    ...
    private Mono<Void> executeStep(String sagaName,
        ...
        if (sd.handler != null) {
            ...
            execution = attemptCall(saga.bean, invokeMethod, input, ctx, sd.timeoutMs, sd.retry, sd.backoffMs, stepId);
        }
        return execution
...
/**
     ...
     * @param method    method to invoke (proxy-safe)
     ...
     */
    private Mono<Object> attemptCall(Object bean, Method method, Object input, SagaContext ctx,
                                     long timeoutMs, int retry, long backoffMs, String stepId) {
        return Mono.defer(() -> invokeMono(bean, method, input, ctx))
                .transform(m -> timeoutMs > 0 ? m.timeout(Duration.ofMillis(timeoutMs)) : m)
                .onErrorResume(err -> {
                    if (retry > 0) {
                        return Mono.delay(Duration.ofMillis(Math.max(0, backoffMs)))
                                .then(attemptCall(bean, method, input, ctx, timeoutMs, retry - 1, backoffMs, stepId));
                    }
    ...
    private Mono<Object> invokeMono(Object bean, Method method, Object input, SagaContext ctx) {
        try {
            ...
            if (method.getParameterCount() == 2) {
                result = method.invoke(bean, input, ctx);
            } else if (method.getParameterCount() == 1) {
                // Either input or context
                Class<?> p0 = method.getParameterTypes()[0];
                if (p0.isAssignableFrom(SagaContext.class)) {
                    result = method.invoke(bean, ctx);
                } else {
                ...
                throw new IllegalStateException("Unsupported method signature for " + method.getName());
            }
