...
import com.firefly.transactional.annotations.CompensationSagaStep;
 ...
 * <p>
 * Responsibilities:
 * - Scan for beans annotated with {@link com.firefly.transactional.annotations.Saga}.
 * - For each {@link com.firefly.transactional.annotations.SagaStep} method, build a {@link StepDefinition}
...
public class SagaRegistry {
    ...
    private synchronized void ensureScanned() {
        ...
        for (Object bean : allBeans.values()) {
            Class<?> targetClass = AopUtils.getTargetClass(bean);
            for (Method m : targetClass.getMethods()) {
                com.firefly.transactional.annotations.ExternalSagaStep es = m.getAnnotation(com.firefly.transactional.annotations.ExternalSagaStep.class);
                if (es == null) continue;
    ...
    private void validateOneMethodParams(SagaDefinition saga, StepDefinition sd, Method method, boolean compensation) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (com.firefly.transactional.core.SagaContext.class.isAssignableFrom(type)) {
                continue;
            }

            // Known annotations
            var in = p.getAnnotation(com.firefly.transactional.annotations.Input.class);
            if (in != null) {
                // no extra validation (keyed input may or may not exist at runtime)
                continue;
            }
            var fs = p.getAnnotation(com.firefly.transactional.annotations.FromStep.class);
            if (fs != null) {
                String ref = fs.value();
            ...
            var fcr = p.getAnnotation(com.firefly.transactional.annotations.FromCompensationResult.class);
            if (fcr != null) {
            ...
            var fce = p.getAnnotation(com.firefly.transactional.annotations.CompensationError.class);
            if (fce != null) {
            ...
            var h = p.getAnnotation(com.firefly.transactional.annotations.Header.class);
            if (h != null) {
            ...
            var hs = p.getAnnotation(com.firefly.transactional.annotations.Headers.class);
            if (hs != null) {
            ...
            var varAnn = p.getAnnotation(com.firefly.transactional.annotations.Variable.class);
            if (varAnn != null) {
                // no static type validation; runtime variable value must be assignable to the parameter type
                continue;
            }
            var varsAnn = p.getAnnotation(com.firefly.transactional.annotations.Variables.class);
            if (varsAnn != null) {
                if (!java.util.Map.class.isAssignableFrom(type)) {
