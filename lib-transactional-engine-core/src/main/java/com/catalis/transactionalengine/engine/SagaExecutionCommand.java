...
public class SagaExecutionCommand {
                ...
                .doOnNext(res -> {
                    ...
                    if (sd.stepMethod != null) {
                        var setVarAnn = sd.stepMethod.getAnnotation(
                                com.catalis.transactionalengine.annotations.SetVariable.class);
                        if (setVarAnn != null) {
    ...
    private Mono<Void> publishStepEventIfConfigured(SagaExecutionContext executionContext, String stepId) {
        ...
        if (stepDef == null || stepDef.stepEvent == null || !stepDef.stepEvent.enabled 
                || stepDef.stepEvent.topic == null || stepDef.stepEvent.topic.isBlank()) {
            return Mono.empty();
        }
